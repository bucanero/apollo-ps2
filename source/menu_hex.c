...
static void _draw_HexEditor(const hexedit_data_t* hex, u8 alpha)
{
    ...
    for (i = hex->start; i < hex->size && (i - hex->start) < (MAX_LINES*16); i++)
    {
        if (i != hex->start && !(i % 16))
        {
            DrawFormatStringMono(MENU_ICON_OFF -20, y_off, "%06X \xB3 %s \xB3 %s", i-0x10, msgout, ascii);
            y_off += FNT_HEIGHT;
        }
    ...
    DrawFormatStringMono(MENU_ICON_OFF -20, y_off, "%06X \xB3 %-48s \xB3 %s", (i-1) & ~15, msgout, ascii);
