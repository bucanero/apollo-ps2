    ...
    while ((option_name = menu_options[ind].name))
    {
		...
		switch (menu_options[ind].type)
		{
			...
			case APP_OPTION_CALL:
				DrawTextureCenteredX(&menu_textures[footer_ico_cross_png_index], OPTION_ITEM_OFF, y_off+4, 0, menu_textures[footer_ico_cross_png_index].width /2, menu_textures[footer_ico_cross_png_index].height /2, 0xFFFFFF00 | alpha);
				break;
